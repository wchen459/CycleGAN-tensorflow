...
def load_test_data(image_path, fine_size=256):
    ...
    img = np.array(Image.fromarray(img.astype(np.uint8)).resize([fine_size, fine_size]))
    img = img/127.5 - 1
    return img
...
def load_train_data(image_path, load_size=286, fine_size=256, is_testing=False):
    ...
    if not is_testing:
        img_A = np.array(Image.fromarray(img_A.astype(np.uint8)).resize([load_size, load_size]))
        img_B = np.array(Image.fromarray(img_B.astype(np.uint8)).resize([load_size, load_size]))
